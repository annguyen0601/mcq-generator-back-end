...
class MCQGenerator():
    def __init__(self, is_verbose=False):
        ...
        print('Loaded QuestionGenerator in', round(
            time.perf_counter() - start_time, 2), 'seconds.') if is_verbose else ''

        self.distractor_generator = DistractorGenerator()
        print('Loaded DistractorGenerator in', round(
            time.perf_counter() - start_time, 2), 'seconds.') if is_verbose else ''

        self.sense2vec_distractor_generator = Sense2VecDistractorGeneration()
        print('Loaded Sense2VecDistractorGenerator in', round(
            time.perf_counter() - start_time, 2), 'seconds.') if is_verbose else ''
    ...
    def generate_mcq_questions(self, context: str, desired_count: int) -> List[Question]:
        ...
        questions = self._generate_question_answer_pairs(
            cleaned_text, desired_count)
